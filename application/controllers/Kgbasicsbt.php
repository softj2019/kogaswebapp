...
//		$user_data = $this->common->select_row('member','',Array('email'=>@$this->session->userdata('email')));
		...
		$config['base_url'] =base_url('kgbasicsbt/writeform');
		$user_id = @$this->session->userdata('user_id');
		$config['total_rows'] = $this->common->select_count('(select * from kgartSbtView where analysis_type = \'A\''.
...
//			"(select Z.htm4 from kgrct Z where Z.ar_cd = TB.ar_cd) as htm4" .
		...
		$data["list"]= $this->common->select_list_table_result('(select * from kgartSbtView where analysis_type = \'A\''.
